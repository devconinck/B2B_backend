...
exports.processMessage = async (message: string, state: any) => {
  ...
  switch (responseObj.intent) {
    case "orderStatus":
      if (state == undefined) {
        answer =
          "Hmm, it looks like you are not logged in. Please login to see information about your invoices";
        ...
        const order = await repositoryOrders.findOrder(
          state.role,
          state.companyId,
          orderid
        );
  ...
  return (
    answer || "Sorry! I could not understand that, requesting you to rephrase!"
  );
};
